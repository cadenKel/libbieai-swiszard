...
from .db import get_connection, init_db, get_all_examples, insert_example, increment_success, increment_fail
...
HELP_TEXT = (
    ...
    "\nSHELL\n"
    "  run COMMAND   (wrap COMMAND in backticks)\n"
    "\nWEB\n"
    "  search the web for QUERY\n"
    "\nMEMORY (swiszmem)\n"
    "  memory recall QUERY | memory recall+history QUERY\n"
...
def swiszard_do(task: str, dry_run: bool = False) -> str:
    ...
    if not top:
        ...
        return (
            "swiszard: no handler match yet. Rephrase the task using one of the "
            "supported formats (run `...`, read /path, find ... in /path, "
            "search the web for ..., memory recall|remember|forget|status)."
        )
    else:
        best = top[0]
        narrate(
        ...
        if (
            ...
            narrate(f"no confident match (best sim={best['sim']:.3f}); returning no-match")
            return (
                "swiszard: no confident handler match. Rephrase the task using one of the "
                "supported formats (run `...`, read /path, find ... in /path, "
                "search the web for ..., memory recall|remember|forget|status)."
            )
